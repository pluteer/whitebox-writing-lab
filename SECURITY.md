...
`v0.4.3` 起，Windows 安装包、便携包和 `SHA256SUMS` 使用 Sigstore keyless signing。签名身份由 GitHub Actions OIDC 和 Fulcio 短期证书绑定到本仓库的 `.github/workflows/release.yml`，并写入透明日志。每个发布文件必须同时带有 `<filename>.sigstore.json`；缺少 bundle 的文件不是正式发布产物。

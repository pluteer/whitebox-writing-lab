...
```powershell
.\installer\BuildPortable.ps1 -Version 0.4.1
```

将 `packaging\whitebox-writing-portable-0.4.1.zip` 发送给朋友。对方解压后双击 `StartWhiteboxPortable.bat` 即可打开 WebUI。首次使用仍需在 WebUI 中填写对方自己的模型 API Key；不要分享包含个人数据库或密钥的 `data` 目录。

...
双击 `启动Whitebox.bat` 打开本地 GUI 启动器。当前版本为 `0.4.4`。
...
```powershell
.\installer\构建便携包.ps1 -Version 0.4.4
```

输出为 `packaging\whitebox-writing-portable-0.4.4.zip`。解压后双击 `StartWhiteboxPortable.bat`，不需要 WSL、Python、Node.js 或 npm；首次使用仍需要在 WebUI 中配置朋友自己的模型 API Key。
